...
fn parse_year_core(year: &str) -> MyResult<i32> {
...
fn parse_year(year: &String) -> MyResult<i32> {
    parse_year_core(year)
}

fn parse_month_core(month: &str) -> MyResult<u32> {
...
#[cfg(test)]
mod tests {
    use super::{format_month, last_day_in_month, parse_int, parse_month_core, parse_year_core};
    ...
    #[test]
    fn test_parse_year() {
        let res = parse_year_core("1");
        assert!(res.is_ok());
        assert_eq!(res.unwrap(), 1i32);

        let res = parse_year_core("9999");
        assert!(res.is_ok());
        assert_eq!(res.unwrap(), 9999i32);

        let res = parse_year_core("0");
        assert!(res.is_err());
        assert_eq!(
    ...
    #[test]
    fn test_parse_month() {
        let res = parse_month_core("1");
        assert!(res.is_ok());
        assert_eq!(res.unwrap(), 1u32);

        let res = parse_month_core("12");
        assert!(res.is_ok());
        assert_eq!(res.unwrap(), 12u32);

        let res = parse_month_core("jan");
        assert!(res.is_ok());
        assert_eq!(res.unwrap(), 1u32);

        let res = parse_month_core("0");
        assert!(res.is_err());
        assert_eq!(
            res.unwrap_err().to_string(),
